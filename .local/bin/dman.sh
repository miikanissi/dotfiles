...
if [ "$prompt" = mount ]; then
  dev="$(lsblk -lp | grep -E "/dev/.*part" | awk '{print $1}' | dmenu -p "mount")"
  [ -z "$dev" ] && exit 0;
  ...
  dev=$(lsblk -lp | grep -E "/dev/.*part|/dev/.*crypt" | awk '{print $1}' | dmenu -p "umount")
  [ -z "$dev" ] && exit 0;
  if ! mount | grep -q "$dev"; then
  ...
  dev=$(lsblk -lp | grep -E "/dev/.*disk" | awk '{print $1}' | dmenu -p "eject")
  [ -z "$dev" ] && exit 0;
  sudo eject "$dev"
