...
#+end_src
* Email
Most email clients are slow, ugly, bloated, lack features or all of the above so using email in Emacs seems like a good solution. Most email is sent as text/plain in addition to text/html anyway so having to render HTML shouldn't be a problem. My configuration includes a hotkey to open a HTML email in a browser as well.
