...
- =wmctrl=, important! this small program detects if WM is running, if not then EXWM is used.
...
#+end_src
* Desktop Environment
Everything regarding the WM functionality is under this section. To run Emacs as a window manager you need to create the following desktop entry in =/usr/share/xsessions/= directory.
#+begin_src bash
[Desktop Entry]
Name=EXWM
...
#+end_src
* Email *SOON*
Most email clients are slow, ugly, bloated, lack features or all of the above so using email in Emacs seems like a good solution. Most email is sent as text/plain in addition to text/html anyway so having to render HTML shouldn't be a problem.
