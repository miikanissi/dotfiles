...
#+end_src
** Font
Use Ubuntu Mono Nerd Font Mono. Installation script for the font available on my [[https://github.com/miikanissi/dotfiles/blob/master/.local/bin/install_nerd_fonts_ubuntu_mono.sh][GitHub]].
#+begin_src emacs-lisp
  (when (eq system-type 'gnu/linux)
        (add-to-list 'default-frame-alist
                     '(font . "UbuntuMonoNerdFont 12")))
#+end_src
** All-the-icons
...
#+end_src
** Delete process
Delete a process with =ido-completing-read=
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-c C-x") 'delete-process-interactive)
#+end_src
* Terminal
...
#+begin_src emacs-lisp
  ...
  #+end_src
** IDO
Ido mode improves buffer switching and prompts. Only used if Ivy mode is disabled.
*** enable ido mode
#+begin_src emacs-lisp
...
#+end_src
** Ivy
Ivy ensures Emacs commands use Ivy for completion, makes finding files and switching buffers faster.
#+begin_src emacs-lisp
  (use-package ivy
    :ensure t
    :init
...
#+end_src
** Councel
Councel replaces common Emacs commands with a customized Ivy optimized alternatives.
#+begin_src emacs-lisp
  (use-package counsel
    :ensure t
    :init
...
#+end_src
** Odoo
Some helpful functions to improve Odoo development.
*** run-odoo
Defines a function that prompts for odoo version to run and a name of a module to update. NOTE: If you copy this you need to change your own file paths.
#+begin_src emacs-lisp
  (defun run-odoo ()
    (interactive)
    (let ((version-list '("12" "14")))
      (let ((version (ido-completing-read "Select Odoo version: " version-list)))
        (let ((update (ido-completing-read "Select module to update: " (directory-files (concat "/opt/odoo/" version "/addons/mpg-dev-linked-all")))))
          (let ((odoo (concat "/opt/odoo/" version "/server/odoo-bin")))
            (let ((conf (concat "/opt/odoo/" version "/conf/odoo-" version "-all-dev-modules.conf")))
              (start-process "odoo" "odoo" odoo "-c" conf "-u" update)))))))
  (global-set-key (kbd "C-c C-o") 'run-odoo)
#+end_src
* Org
